...
def scale_data(df_train: pd.DataFrame, df_test: pd.DataFrame):
    """
    MinMax-skalowanie cech numerycznych:
    - usuwamy kolumnę 'price' z cech
    - brakujące wartości zastępujemy zerami
    """
...
def automl_train(X_train: pd.DataFrame,y_train: pd.Series,time_budget: int = 300) -> object:
    """
    Trenowanie FLAML AutoML:
    - time_budget: limit czasu w sekundach
    - optymalizujemy RMSE w zadaniu regresji
    """
    automl = AutoML()
    automl_settings = {
...
def evaluate_model(model: object, X_test: pd.DataFrame, y_test: pd.Series) -> pd.DataFrame:
    """
    Ewaluacja wybranego modelu:
    - obliczamy MAE, RMSE i R^2 na zbiorze testowym
    - zwracamy wynik w formie jednorowkowego DataFrame
    """
